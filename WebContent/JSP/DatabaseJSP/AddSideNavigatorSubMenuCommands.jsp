...
<%
		...
		strHeader = "Add Top Navigator Sub Menu Commands";
	
		Actions action = new Actions();	
		int count = 0;
		try
		{
		CachedRowSet resultSetCheckAvailableCommands = action.fetchDataFromDB("clientsidenavigatorsubmenucommand", null, "Name != 'NA'");
...
%>
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				if(checkCount == true)
					{
						$('#Add_Top_Navigator_linksTable').html("<br><br><h6>Maximum number of Commands already added!</h6>");
					}
			});
			
			</script>
		
		<title>Top Navigator links</title>
	</head>
	...
	<body>
		...
		<div id="Add_Top_Navigator_linksDiv" class="Add_Top_Navigator_linksDiv">

			<form id="Add_Top_Navigator_linksForm" name="Add_Top_Navigator_linksForm" method="POST" action="/Choice/JSP/DatabaseJSP/AddSideNavigatorSubMenuCommandsProcess.jsp" enctype="multipart/form-data" onsubmit="showProgress();">

				

				<input type="hidden" id="DatabaseTableName" name="DatabaseTableName" value="clientsidenavigatorsubmenucommand" />

				
				<table id="Add_Top_Navigator_linksTable" name="Add_Top_Navigator_linksTable" border="1" bgcolor="#e5e5e5">
...
<%
							...
							CachedRowSet resultSetAvailableForms = action.fetchDataFromDB("formmaster", null, strWhere);
							CachedRowSet resultSetAvailableDynamicTables = action.fetchDataFromDB("tablemaster", null, strWhere);

								while(resultSetAvailableForms.next())
								{
									
								String strEachFormName = resultSetAvailableForms.getString("Name");
...
%>
						...
						<td>
							<select id="MenuName" name="MenuName" required>
<%
								strWhere = "Name != 'NA'";

								CachedRowSet resultSetAvailableParentMenus = action.fetchDataFromDB("clientsidenavigatorsubmenu", null, strWhere);
								while(resultSetAvailableParentMenus.next())
									{
										String strEachParentMenuName = resultSetAvailableParentMenus.getString("Name");
	


%>							
											<option value="<%=strEachParentMenuName%>"><%=strEachParentMenuName%> - Menu</option>
<%
...
%>
								...
								<input type="hidden" id="MenuNameDatabaseTableFieldName" name="MenuNameDatabaseTableFieldName" value="MenuName" />
					
				</table><br>


				<input type="submit" id="Add_Top_Navigator_linksSubmit" name="Add_Top_Navigator_linksSubmit" value="Add Commands" />
...
<%
		...
		{
			System.out.println("Exception in AddTopNavigatorLinks.jsp::"+e);
%>
