...
<%
		...
		String DBTableName = "clientsidenavigator";
		...
		try
		{
				for(int i=0;i<SelectedFields.size();i++)
				{
					...
					if(resultSetSideNavigator.next())
					{
						...
						IsMenu = resultSetSideNavigator.getString("IsMenu");
					}
					if(strPrimaryKey != null)
					{
						actionDone = action.DeleteExistingEntry(DBTableName, strPrimaryKey);
					}
					if(IsMenu.trim().equalsIgnoreCase("true"))
					{
						CachedRowSet resultSetSideNavigatorSubMenu = action.fetchDataFromDB("clientsidenavigatorsubmenu", null, strWhere);
						if(resultSetSideNavigatorSubMenu.next())
						{
							strPrimaryKey = resultSetSideNavigatorSubMenu.getString("clientsidenavigatorsubmenu_UID");
						}
						if(strPrimaryKey != null)
						{
							actionDone = action.DeleteExistingEntry("clientsidenavigatorsubmenu", strPrimaryKey);
						}
					}
				}
...
%>
						<script>
							window.alert("Menus/Commands are removed Successsfully!");
