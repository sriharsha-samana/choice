		...
		function createFields()
			{
				...
				var fieldTypes = ["text","password","select","radio","checkbox","number","textarea","date","email","range","url"];
				var HTML = "<table border=1>";
				HTML += "<th>Type</th><th>Name</th><th>Label</th><th>Options</th><th>Validations</th><th>Field Name</th>";
				for(var i=0;i<NoOfFields;i++)
					{
						...
						HTML += "<td id=\"DynamicFieldTableTDIDOptions"+i+"\"><input type=\"text\" id=\"Options"+i+"\" name=\"Options"+i+"\" value=\"NA\" readonly required /></td><td id=\"DynamicFieldTableTDIDValidations"+i+"\">";
						HTML += "<input type=\"text\" id=\"Validations"+i+"\" name=\"Validations"+i+"\" value=\"NA\" required />";
						HTML += "</td><td id=\"DynamicFieldTableTDIDDataBaseField"+i+"\">";
		...
		function updateOptions(count)
			{
				...
				if(selectedType == "select" || selectedType == "radio" || selectedType == "checkbox")
					{
						var options = window.prompt("Enter number of Options required", "1");
						if(options != null)
							{
								var check = checkForNumber(options);
								if(check)
									{
										var HTML = "<input type=\"hidden\" id=\"OptionsCount"+count+"\" name=\"OptionsCount"+count+"\" value=\""+options+"\"><table border=0 width=100%>";
										HTML += "<tr>";
										for(i=0;i<options;i++)
											{
												HTML += "<td>";
												HTML += "<input type=\"text\" id=\"Options"+count+"Internal"+i+"\" name=\"Options"+count+"Internal"+i+"\" required/>";
												HTML += "</td>";
											}
										HTML += "</tr>";
										HTML += "</table>";
										document.getElementById("DynamicFieldTableTDIDOptions"+count).innerHTML = HTML;
									}
							}
					}
				else if(selectedType == "number" || selectedType == "range")
				{
					window.alert("Enter maximum and minimum ranges in Options");
					var HTML = "<input type=\"hidden\" id=\"OptionsCount"+count+"\" name=\"OptionsCount"+count+"\" value=\"2\"><table border=0 width=100%>";
					HTML += "<tr>";
					for(i=0;i<2;i++)
						{
							HTML += "<td>";
							HTML += "<input type=\"number\" id=\"Options"+count+"Internal"+i+"\" name=\"Options"+count+"Internal"+i+"\" required/>";
							HTML += "</td>";
						}
					HTML += "</tr>";
					HTML += "</table>";
				...
				else if(selectedType == "hyperlink" || selectedType == "image")
				{
					window.alert("Enter href and src values in Options. Enter \"NA\" if not applicable.");
					var HTML = "<input type=\"hidden\" id=\"OptionsCount"+count+"\" name=\"OptionsCount"+count+"\" value=\"2\"><table border=0 width=100%>";
					HTML += "<tr>";
					for(i=0;i<2;i++)
						{
					...
					{
						document.getElementById("DynamicFieldTableTDIDOptions"+count).innerHTML = "<td><input type=\"text\" id=\"Options"+count+"\" name=\"Options"+count+"\" value=\"NA\" readonly/></td>";
					}
			}
...
<%
			String strHeader = "Create Dynamic Form";
			if(isWriteJSP.equals("true"))
			{
				strHeader = "Write Form JSP";
			}
%>
		...
		<div class="ActiveScreenObj" id="ActiveScreenObj" name="ActiveScreenObj">
			...
			<div id = CreateDynamicFormDiv> 
				...
				<form id="CreateDynamicForm" method="post" action="/Choice/JSP/DatabaseJSP/CreateDynamicFormProcess.jsp" onsubmit="return verifyInput(); showProgress();">
...
%>
							...
							<tr>
								<td>
									Success message after form processing:
								</td>
								<td>
									<input type="text" id="SuccessMessage" name="SuccessMessage" value="NA" required/>
								</td> 
							</tr>	
